...
if __name__ == "__main__":
    ...
    # Cambiar al directorio del README para que las rutas relativas funcionen
    original_dir = os.getcwd()
    readme_dir = os.path.dirname(os.path.abspath(readme_path))
    readme_filename = os.path.basename(readme_path)
    
    os.chdir(readme_dir)
    
    try:
        data = parse_readme_md(readme_filename)
        output_name = data["OUTPUT_NAME"]

        output_tex = f"build/{output_name}.tex"
        os.makedirs("build", exist_ok=True)

        # Usar la ruta del template desde el directorio original
        template_path = os.path.join(original_dir, "product_brief_template.tex")
        render_latex(template_path, output_tex, data)
        compile_pdf(output_tex)
        clean_aux_files(f"build/{output_name}")
        
        print(f"✅ PDF generado: {readme_dir}/build/{output_name}.pdf")
